#!/bin/bash
#
# Unified test script for backup and restore from S3 with optional encryption.
#
# In the below we start a small FDB cluster, populate it with
# some data and then start up a seaweedfs instance or use S3
# if it is available. We then run a backup to 'S3' (with optional encryption)
# and then a restore, verifying the restore is the same as the original.
#
# Usage:
...
# Create an encryption key file for testing
...
function backup {
  ...
  # Add encryption key file argument if provided
...
# Check if encryption was used by examining the logs for encryption messages
# $1 The scratch directory to examine logs
function verify_encryption {
  ...
  if grep -q "encryption-key-file" "${local_scratch_dir}"/*.log || 
     ...
     grep -q "StreamCipherKey" "${local_scratch_dir}"/*.log; then
    log "Encryption verification successful: Found encryption-related messages in logs"
    return 0
  else
    err "Failed to verify encryption: No encryption-related messages found in logs"
    return 1
  fi
}
...
function test_s3_backup_and_restore {
  ...
  log "Run backup" $([[ -n "${local_encryption_key_file}" ]] && echo "with encryption" || echo "")
  ...
  log "Restore from backup" $([[ -n "${local_encryption_key_file}" ]] && echo "with encryption" || echo "")
  ...
  # If encryption key file was provided, verify encryption was used
  if [[ -n "${local_encryption_key_file}" ]]; then
...
readonly TAG="test_backup${USE_ENCRYPTION:+_encryption}"
...
if [[ "${USE_S3}" == "true" && "${USE_ENCRYPTION}" != "true" ]]; then
  # Use AWS KMS encryption for real S3 when not using file encryption
  KNOBS=("--knob_blobstore_encryption_type=aws:kms" "--knob_http_verbose_level=${HTTP_VERBOSE_LEVEL}")
else
  # No encryption for SeaweedFS or when using file encryption
...
if [[ "${USE_S3}" == "true" ]]; then
  log "Testing against s3" $([[ "${USE_ENCRYPTION}" == "true" ]] && echo "with file encryption" || echo "")
  ...
  log "Testing against seaweedfs" $([[ "${USE_ENCRYPTION}" == "true" ]] && echo "with file encryption" || echo "")
...
test="test_s3_backup"
if [[ "${USE_ENCRYPTION}" == "true" ]]; then
  test="${test}_encryption"
fi
url="blobstore://${host}/${path_prefix}/${test}?${query_str}"
log "Using URL: ${url}"

if ! test_s3_backup_and_restore "${url}" "${TEST_SCRATCH_DIR}" "${blob_credentials_file}" "${build_dir}" "${ENCRYPTION_KEY_FILE}"; then
  log_test_result 1 "${test}"
  exit 1
else
  log_test_result 0 "${test}"
fi
