#!/bin/bash
#
# Test backup and restore from s3 with encryption.
#
# In the below we start a small FDB cluster, populate it with
# some data and then start up a seaweedfs instance or use S3
# if it is available. We then run a backup to 'S3' with encryption
# and then a restore, verifying the restore is the same as the original.
#
# Debugging, run this script w/ the -x flag: e.g. bash -x s3_backup_encryption_test.sh...
...
# Run a backup to s3 and then a restore with encryption.
# $1 The url to use
# $2 the scratch directory
# $3 The credentials file.
# $4 build directory
# $5 encryption key file
function test_s3_backup_and_restore_encryption {
  local local_url="${1}"
  local local_scratch_dir="${2}"
  local credentials="${3}"
...
readonly TAG="test_backup_encryption"
...
# Note: Since we're using --encryption-key-file, we don't use KMS encryption
